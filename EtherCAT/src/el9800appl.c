...
void APPL_InputMapping(UINT16* pData)
{
	...
	for (j = 0; j < sTxPDOassign.u16SubIndex0; j++)
	{
		...
		/* TxPDO 1 */
		case 0x1A00:
			*pTmpData++ = SWAPWORD(((UINT16 *) &sFeedBack)[1]);		//init
			*pTmpData++ = SWAPWORD(((UINT16 *) &sFeedBack)[2]);		//clampint
			*pTmpData++ = SWAPWORD(((UINT16 *) &sFeedBack)[3]);		//position
			*pTmpData++ = SWAPWORD(((UINT16 *) &sFeedBack)[5]);		//current
			*pTmpData++ = SWAPWORD(((UINT16 *) &sFeedBack)[6]);		//error
			break;
//      /* TxPDO 3 */
//      case 0x1A02:
...
void APPL_OutputMapping(UINT16* pData)
{
    ...
    for (j = 0; j < sRxPDOassign.u16SubIndex0; j++)
    {
        ...
        /* RxPDO 2 */
        case 0x1601:
			((UINT16 *) &sBasicControl)[1] = SWAPWORD(*pTmpData++);
			((UINT16 *) &sBasicControl)[2] = SWAPWORD(*pTmpData++);
//			((UINT16 *) &sBasicControl)[3] = SWAPWORD(*pTmpData++);
			((UINT16 *) &sBasicControl)[4] = SWAPWORD(*pTmpData++);
			((UINT16 *) &sBasicControl)[5] = SWAPWORD(*pTmpData++);
            break;
        }
    }
}
